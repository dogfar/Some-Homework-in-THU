...
public interface TacEmitter extends Visitor<FuncVisitor> {
    ...
    default void visitVarSel(Tree.VarSel expr, FuncVisitor mv) {
        if (expr.symbol.isVarSymbol()) {
            ...
            expr.lambda.accept(this, mv);
            expr.val = expr.lambda.val;
        }
    }
    ...
    @Override
    default void visitPreCall(Tree.PreCall expr, FuncVisitor mv) {
    ...
    default void visitLambda(Tree.Lambda lambda, FuncVisitor mv){
        for(var x : lambda.params)x.accept(this, mv);
        var a = emitArrayInit(mv.visitLoad(1 + lambda.symbol.captured.size() + (lambda.symbol.hasThis ? 1 : 0)), mv);
        mv.visitStoreTo(a, 0 , mv.getEntry(lambda.currentClass, lambda.symbol.name));
        var i = 0;
        if(lambda.symbol.hasThis) {
            mv.visitStoreTo(a, 4 * (++i), mv.getArgTemp(0));
        }
        for(var c : lambda.symbol.captured){
            mv.visitStoreTo(a, 4 * (++i), c.temp);
        }
        lambda.val = a;
    }
    /**
     ...
     *
     */
    default void visitCall(Tree.Call lambda, FuncVisitor mv){
        lambda.expr.accept(this, mv);
        for(var x : lambda.args){
            x.accept(this, mv);
        }
        var entry = mv.visitLoadFrom(lambda.expr.val);
        var len = mv.visitBinary(TacInstr.Binary.Op.ADD, mv.visitLoad(-1), mv.visitLoadFrom(lambda.expr.val, -4));
        var i = mv.visitLoad(0);
        var ex = mv.freshLabel();

        Function<FuncVisitor, Temp> test = v -> mv.visitBinary(TacInstr.Binary.Op.LES, i, len);
        Consumer<FuncVisitor> body = v -> {
            mv.visitAssign(i, mv.visitBinary(TacInstr.Binary.Op.ADD, i, mv.visitLoad(1)));
            mv.func.add(new TacInstr.Parm(mv.visitLoadFrom(mv.visitBinary(TacInstr.Binary.Op.ADD, lambda.expr.val, mv.visitBinary(TacInstr.Binary.Op.MUL, i, mv.visitLoad(4))))));
        };
        emitWhile(test, body, ex, mv);
        for(var x : lambda.args){
            mv.func.add(new TacInstr.Parm(x.val));
        }
    ...
    private void emitDivError(Temp value, FuncVisitor mv){
        var zero = mv.visitLoad(0);
        var error = mv.visitBinary(TacInstr.Binary.Op.EQU, value, zero);
        var handler = new Consumer<FuncVisitor>() {
            public void accept(FuncVisitor v) {
