...
public class LLParser extends Phase<InputStream, Tree.TopLevel> {
    ...
    private class Parser extends decaf.frontend.parsing.LLTable {
        ...
        private SemValue parseSymbol(int symbol, Set<Integer> follow) {
            ...
            Set<Integer> beginS = beginSet(symbol);
            Set<Integer> endS = followSet(symbol);
            endS.addAll(follow);
            if (!beginS.contains(token)) {
                haveError = true;
                ...
                while (true) {
                    ...
                    else if (endS.contains(token)) {return null;}
                    token = nextToken();
                }
            }
            ...
            for (var i = 0; i < length; i++) { // parse right-hand side symbols one by one
                ...
                params[i + 1] = isNonTerminal(term)
                        ? parseSymbol(term, endS) // for non terminals: recursively parse it
                        : matchToken(term) // for terminals: match token
                ;
            }

            if(!haveError) act(actionId, params); // do user-defined action
            return params[0];
        }
