...
public class Typer extends Phase<Tree.TopLevel, Tree.TopLevel> implements TypeLitVisited {
    ...
    public void visitLambda(Tree.Lambda lamb, ScopeStack ctx){
        ctx.lambdanum ++;
        Type returnType;
        ctx.open(lamb.symbol.scope);
        var argTypes = new ArrayList<Type>();
        for(var param:lamb.params){
            param.accept(this, ctx);
            argTypes.add(param.typeLit.type);
        }
        if(!lamb.ifdev){
            lamb.body.accept(this,ctx);
            returnType = lamb.symbol.scope.getbound();
            if(!returnType.eq(BuiltInType.VOID) && !(lamb.body.stmts.get(lamb.body.stmts.size()-1).returns)){
                issue(new MissingReturnError(lamb.body.pos));
            }
            if(!returnType.noError()){
                issue(new IncompatReturnError(lamb.body.pos));
            }
        }
        else{
            ctx.open(lamb.symbol.scope.nestedLocalScope());
            lamb.obj.accept(this, ctx);
            returnType = lamb.obj.type;
            ctx.close();
        }

        if(!returnType.noError()){
            lamb.type = BuiltInType.ERROR;
        }
        else {
            lamb.type = new FunType(returnType, argTypes);
            lamb.symbol.type = (FunType) lamb.type;
        }
        ctx.Capture();
        ctx.close();
        ctx.lambdanum --;
    ...
    public void visitBlock(Tree.Block block, ScopeStack ctx) {
        ...
        for (var stmt : block.stmts) {
            ...
            if(ctx.lambdanum > 0 && (stmt instanceof Tree.Assign)) {
                var lhs = ((Tree.Assign) stmt).lhs;
                if ((lhs instanceof Tree.VarSel)) {
                    if (((Tree.VarSel) lhs).receiver.isEmpty()) {
                        var lname = (((Tree.VarSel) lhs).variable.name);
                        var ans = ctx.lookupOutsideLambda(lname, stmt.pos);
                        if (ans.isPresent()) {
                            issue(new LambdaCapturedAssignError(stmt.pos));
                        }
                    }
                }
            }
    ...
    public void visitAssign(Tree.Assign stmt, ScopeStack ctx) {
        ...
        if(stmt.lhs instanceof Tree.VarSel){
            if(((Tree.VarSel)stmt.lhs).ifmethod){
                issue(new AssignMethodError(stmt.pos, ((Tree.VarSel) stmt.lhs).name));
            }
        }
    ...
    public void visitReturn(Tree.Return stmt, ScopeStack ctx) {
        if (ctx.lambdanum>0){
            ...
            var ls = (LambdaScope)ctx.currentlambdascope();
            ls.returnTypes.add(actual);
    ...
    public void visitVarSel(Tree.VarSel expr, ScopeStack ctx) {
        visitPreVarSel(expr, ctx);
        if(expr.type.noError() && expr.ifmethod){
            var args = new ArrayList<Tree.Expr>();
            var params = new ArrayList<Tree.LocalVarDef>();
            for (var c : ((MethodSymbol) expr.symbol).args) {
                params.add(new Tree.LocalVarDef(c.typeLit, new Tree.Id("expr@" + expr.pos + "." + c.name, Pos.NoPos), Pos.NoPos, Optional.empty(), Pos.NoPos));
                args.add(new Tree.VarSel(Optional.empty(), new Tree.Id("expr@" + expr.pos + "." + c.name, Pos.NoPos), Pos.NoPos));
            }
            var lamb = new Tree.Lambda(params, new Tree.PreCall(expr.receiver, expr.variable, args, Pos.NoPos),expr.variable.pos);
            expr.lambda = lamb;
            lamb.accept(namer, ctx);
            lamb.accept(this, ctx);
            expr.symbol = lamb.symbol;
        }
    }

    public void visitPreVarSel(Tree.VarSel expr, ScopeStack ctx) {
        ...
        if (expr.receiver.isEmpty()) {
            ...
            if (symbol.isPresent()) {
                if (symbol.get().isVarSymbol()) {
                    if(ctx.av.contains(symbol.get())){
                        expr.type = BuiltInType.ERROR;
                        issue(new UndeclVarError(expr.pos, expr.name));
                        return;
                    }
                    var var = (VarSymbol) symbol.get();
                    Log.info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"+var+expr.pos);
                    expr.symbol = var;
                    expr.type = var.type;
 //                   Log.info("------------"+expr.name+expr.pos+symbol+expr.type);
                    if (var.isMemberVar()) {
                        if (ctx.currentMethod().isStatic()) {
        ...
        if (receiver instanceof Tree.VarSel) {
            ...
            if (v1.isClassName) {
                var symbol = ctx.getClass(v1.name).scope.lookup(expr.name);
                if(symbol.isPresent() && symbol.get().isMethodSymbol()){
                    var msymbol = (MethodSymbol)symbol.get();
                    expr.symbol = msymbol;
                    if(!msymbol.isStatic()){
                        issue(new NotClassFieldError(expr.pos, expr.name, ctx.getClass(v1.name).type.toString()));
                        return;
                    }
                }else if(symbol.isPresent() && symbol.get().isVarSymbol()){
                    issue(new NotClassFieldError(expr.pos, expr.name, ctx.getClass(v1.name).type.toString()));
                    return;
                }
        ...
        if(rt.isArrayType() && expr.variable.name.equals("length")){
            ...
            expr.symbol = new VarSymbol("length", expr.type, expr.pos);
            return;
        }
    ...
    public void visitCall(Tree.Call expr, ScopeStack ctx) {
        var clazz = ctx.currentClass();
        expr.expr.accept(this, ctx);
        expr.type = expr.expr.type;
 //       if(expr.expr instanceof Tree.Call){
  //      Log.info("qwqwqwqw"+expr.pos+((Tree.Call)expr.expr).expr+"*******"+expr.type);}
        Log.info("___________"+expr.pos+expr.expr+expr.expr.type);

        if(expr.expr instanceof Tree.VarSel) {
            expr.receiver = ((Tree.VarSel) expr.expr).receiver;
            expr.methodName = ((Tree.VarSel) expr.expr).variable.name;
            if (expr.receiver.isPresent() && !(expr.receiver.get() instanceof Tree.This)) {
                var receiver = expr.receiver.get();
                if (receiver instanceof Tree.VarSel && ((Tree.VarSel)receiver).isClassName) {
                    clazz = ctx.getClass(((Tree.VarSel)receiver).name);
                }
                else if(receiver.type.isClassType()) {
                    clazz = ctx.getClass(((ClassType)receiver.type).name);
                }
            }

            var symbol = clazz.scope.lookup(expr.methodName);
            if(symbol.isPresent() && symbol.get().isMethodSymbol()){
                expr.symbol = (MethodSymbol)symbol.get();
            }
        }

        if(expr.type == null)return;
        if(!expr.type.noError())return;
        if(!expr.type.isFuncType()){
            issue(new NotCallableError(expr.pos, expr.type.toString()));
            expr.type = BuiltInType.ERROR;
            return;
        }
        ...
        if (((FunType)expr.type).arity() != args.size()) {
            if(expr.methodName == null){
                issue(new LambdaArgCountError(expr.pos, ((FunType)expr.type).arity(), args.size()));
            }
            else{
                issue(new BadArgCountError(expr.pos, expr.methodName, ((FunType)expr.type).arity(), args.size()));
            }
        }
        var iter1 = ((FunType)expr.type).argTypes.iterator();
        ...
        expr.type = ((FunType) expr.type).returnType;
        Log.info("LALALA  "+expr.toString()+" "+expr.type.toString());
    }
    ...
    public void visitLocalVarDef(Tree.LocalVarDef stmt, ScopeStack ctx) {
        if (stmt.initVal == null){
            ...
            if(rt.eq(BuiltInType.VOID)){
                issue(new BadVarTypeError(stmt.id.pos, stmt.name));
                return;
            }
            stmt.symbol.type = rt;
 //           Log.info("----------"+rt+stmt.pos);
            return;
        }
    ...
    public void visitPreCall(Tree.PreCall expr, ScopeStack ctx) {
        expr.type = BuiltInType.ERROR;
        Type rt;
        ...
        Log.info("visitPreCall!!"+expr.receiver+"  "+expr.methodName+" !!type!! "+expr.type);
    }
    private void typeCall(Tree.PreCall call, boolean thisClass, String className, ScopeStack ctx, boolean requireStatic) {
        ...
        if (symbol.isPresent()) {
            if (symbol.get().isMethodSymbol()) {
                ...
                if (requireStatic && !method.isStatic()) {
                    if(call.expr instanceof Tree.VarSel){
                        issue(new NotClassFieldError(((Tree.VarSel) call.expr).variable.pos, call.methodName, clazz.type.toString()));
                    }
                    else {
            ...
            if(call.receiver.get().type != null){
                issue(new FieldNotFoundError(((Tree.VarSel) call.expr).variable.pos, call.methodName, clazz.type.toString()));
            }
            else{
                issue(new UndeclVarError(((Tree.VarSel) call.expr).variable.pos, call.methodName));
            }
        }
    }
}
