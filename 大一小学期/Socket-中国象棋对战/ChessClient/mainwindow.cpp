...
void MainWindow::settingit(){
    ...
    connect(cd,SIGNAL(connectit()),this,SLOT(connecthost()));
    cd->show();
}
...
void MainWindow::newgame(){
    ...
    yourcolor=qrand()%2;
    if(yourcolor==0){
        this->setWindowTitle("Chinese chess game Client-您执红方");
        array->append("new0");
    }
    else {this->setWindowTitle("Chinese chess game Client-您执黑方");array->append("new1");}
    a=30;
    chosenitem.c_p=QPoint(-1,-1);
    redturn = true;
    existing = false;
...
void MainWindow::readfile(){
    ...
    if(yourcolor==0)this->setWindowTitle("Chinese Chess Client-您执红方");
    else this->setWindowTitle("Chinese Chess Client-您执黑方");
    update();
}
void MainWindow::connecthost(){
    QString s=cd->le1->text();
    this->readwritesocket = new QTcpSocket;
    this->readwritesocket->connectToHost(QHostAddress(s),8888);
