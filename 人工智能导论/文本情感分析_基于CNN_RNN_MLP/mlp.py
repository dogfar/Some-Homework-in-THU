...
checkpointer = ModelCheckpoint(filepath="weights-simmlp.hdf5", verbose=1, save_best_only=True,mode='min')
w=embedding.ite
model=Sequential()
model.add(Embedding(vocab_size,EMBEDDING_SIZE,input_length=1500,weights=[ite],trainable=False))
model.add(Flatten())
...
##FOR TEST
##model.load_weights(filepath="E:/sinanews/sina/weights-simmlp.hdf5")
