...
FieldList       :   STATIC Type Id '(' VarList ')' Block FieldList
                ...
                |   ABSTRACT Type Id '(' VarList ')' ';' FieldList
                    {
                        $$ = $8;
                        $$.fieldList.add(0, new MethodDef($3.id, $2.type, $5.varList, $3.pos));
                    }
...
Type            :   AtomType ArrayType
                    {
                        ...
                        for(var sv : $2.thunkList){
                            if(sv.typeList != null) {
                                $$.type = new TLambda($1.type, sv.typeList, $1.type.pos);
                            }
                            else {
...
SimpleStmt      :   Var Initializer
                ...
                |   VAR Id '=' Expr
                    {
                        $$ = svStmt(new LocalVarDef($2.id, $3.pos, $4.expr, $2.pos));
                    }
                |   /* empty */
                    {
                        $$ = svStmt(null);
                ...
                |   AtomType T '[' AfterLBrack
                    {
                        ...
                        for (var sv : $4.thunkList){
                            if(sv.typeList != null){
                                $$.type = new TLambda($1.type, sv.typeList, $1.type.pos);
                            }
                            else {$$.type = new TArray($1.type, $1.type.pos);}
                        }
                        $$.expr = $4.expr;
                    }
                ;
AfterLBrack     :   ']' T '[' AfterLBrack
                    {
                        $$ = $4;
